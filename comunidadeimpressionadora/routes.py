...
lista_usuarios = ['Vitor', 'Manu', 'Laura']
...
def login():
    ...
    if form_criarconta.validate_on_submit() and 'botao_criar_conta' in request.form:
        ...
        usuario = Usuario(username=form_criarconta.nome.data, email=form_criarconta.email.data, senha=senha_cript)
...
def sair():
    logout_user()
    flash(f'Logout realizado com sucesso. Entre ou cadastre-se.', "alert-success")
    return redirect(url_for('home'))
